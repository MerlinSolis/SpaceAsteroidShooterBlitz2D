AppTitle "Vector Bounce and Delta Time"
...
While run
	...
	player_pos_x# = player_pos_x# + (player_speed * player_dir[0])*dt#
	player_pos_y# = player_pos_y# + (player_speed * player_dir[1])*dt#
	
	
	
	If (player_pos_x + player_img_w) > SCREEN_WIDTH Or player_pos_x < 0 Then
		player_dir[0] = player_dir[0] * -1
	EndIf
	If (player_pos_y + player_img_h) > SCREEN_HEIGHT Or player_pos_y < 0 Then
		player_dir[1] = player_dir[1] * -1
	EndIf
